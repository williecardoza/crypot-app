...
export const getCoinList = () => async (dispatch, getState) => {
  ...
  try {
    const coinListURL = `https://api.coingecko.com/api/v3/coins/markets?vs_currency=${state.app.currency}&order=${order ? "market_cap" : "volume"}_desc&per_page=${coinsPerPage}&page=${page}&sparkline=true&price_change_percentage=1h%2C24h%2C7d`;
...
export const updatePage = update => (dispatch, getState) => {
  ...
  dispatch({
    type: UPDATE_PAGE,
    payload: update ? state.coinList.page + 1 : state.coinList.page > 1 ? state.coinList.page - 1 : 1,
  });
};
